...
class CVDALRLSystem:
    def __init__(self, data, config):
        ...
        if self.strategy != "rl":
            ...
            self.agent = GRPO(state_dim, config)
    ...
    def run_al_epoch(self):
        ...
        strategy_weights = np.clip(strategy_weights, 0, None)
        ...
        # get reward
        reward = self._compute_long_term_reward(new_auc)

        done = self.early_stop or (self.labeled_mask.sum() >= self.budget)

        # Store experience, GRPO does not require state values,
        # only reward and termination flags are stored
        self.agent.buffer.rewards.append(reward)
        self.agent.buffer.is_terminals.append(done)

        self.agent.update()

        self._log_training_metrics(strategy_weights, reward)
    ...
    def _log_training_metrics(self, strategy_weights, reward):
        if not hasattr(self, "training_metrics"):
            # Initializing the Metrics Dictionary
        ...
        # Checking for recordable indicators
        metrics_available = len(self.training_metrics["kl_divergence"]) > 0

        # Obtain updated indicators from GRPO
        if hasattr(self.agent, "last_metrics") and metrics_available:
            metrics = self.agent.last_metrics
            self.training_metrics["kl_divergence"].append(metrics.get("kl_div", 0))
            self.training_metrics["policy_loss"].append(metrics.get("policy_loss", 0))
            self.training_metrics["entropy"].append(metrics.get("entropy", 0))

        self.training_metrics["reward"].append(reward)

        if self.iteration % 5 == 0 and metrics_available:
            print(
                f"GRPO Metrics [Iter {self.iteration}]: "
                f"KL Div: {self.training_metrics['kl_divergence'][-1]:.4f} | "
                f"Policy Loss: {self.training_metrics['policy_loss'][-1]:.4f} | "
                f"Entropy: {self.training_metrics['entropy'][-1]:.4f}"
            )
        elif self.iteration % 5 == 0 and not metrics_available:
            print(f"GRPO Metrics [Iter {self.iteration}]: Initializing...")

    def full_training_loop(self):
        results = []
        start_time = time.time()
        print("Starting Active Learning with GRPO...")
        ...
        print("GRPO Metrics: Initializing training...")
        ...
        while not self.early_stop and self.labeled_mask.sum() < self.budget:
            ...
            print(
                ...
                f"Strategy={np.round(epoch_result['strategy_weights'], 2)} | "
                f"Labeled={epoch_result['num_labeled']}/{self.budget}"
            )
            if epoch_result["iteration"] % 10 == 0:
                self._save_intermediate_results()

        duration = time.time() - start_time
        print(f"GRPO training completed, took {duration:.2f} seconds")
    ...
    def _plot_training_metrics(self):
        ...
        if len(self.training_metrics["kl_divergence"]) < 2:
        ...
        # KL dispersion
        plt.subplot(2, 2, 1)
        plt.plot(self.training_metrics["kl_divergence"], "b-")
        ...
        # policy loss
        plt.subplot(2, 2, 2)
        plt.plot(self.training_metrics["policy_loss"], "g-")
        ...
        # entropy
        plt.subplot(2, 2, 3)
        plt.plot(self.training_metrics["entropy"], "m-")
        ...
        # reward
        plt.subplot(2, 2, 4)
        plt.plot(self.training_metrics["reward"], "c-")
        plt.title("Reward")
        plt.xlabel("Iteration")
        plt.grid(True, alpha=0.3)
